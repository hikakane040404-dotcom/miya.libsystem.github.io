...
async function callGAS(command, data = {}) {
    try {
        ...
        return { success: false, message: "ネットワークエラーまたはGAS側の設定ミスです: " + err.toString() };
    }
}
...
async function fetchData(bg) {
    ...
    if (d && d.success) {
        ...
        if (!bg) alert('データの取得に失敗しました: ' + (d ? d.message : "応答なし"));
    }
...
function renderAdmin() {
    const tb = document.getElementById('admin-table'); tb.innerHTML = '';
...
function closeBookDetail() { document.getElementById('detail-modal').style.display = 'none'; }
...
function handleCode(code) {
    ...
    stopScanner();
    const r = document.getElementById('scan-result'); r.innerHTML = '';
    if (b) {
        const ok = b.status === 'available';
