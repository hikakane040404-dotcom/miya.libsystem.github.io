// CONFIG (GASのURL)
const GAS_API_URL = "https://script.google.com/macros/s/AKfycbw20V6S4AePIJDe8FtddGlA9FmLC29okGDjvlWue4TXiUrRw7oHKtksYC51XAAYli-t0w/exec";
...
async function showAppWithSplash() {
    ...
    setTimeout(() => {
        ...
        if (mobNav) mobNav.classList.add('show-anim');
    }, 1100);
...
function applyData(rawBooks, rawUsers, isBg = false) {
    ...
    if (books.length > 0) enrichData(books);
}
 ...
 */
async function enrichData(list) {
    if (!list || list.length === 0) return;

    // API制限対策: セッション中で5分間のクールダウン
    const now = Date.now();
    ...
    // サムネイル or 説明文が欠けている本を抽出 (最大15件程度に制限)
    const targets = list.filter(b => (!b.thumbnail || !b.description) && b.isbn).slice(0, 15);
    if (targets.length === 0) return;

    console.log(`[enrichAPI] ${targets.length}冊の情報不足をサーバー経由で補完します...`);

    const isbns = targets.map(b => String(b.isbn).replace(/[' ]/g, ''));
    const res = await callGAS("bulkEnrichISBNs", { isbns });
    ...
    if (res && res.success && res.results) {
        ...
        targets.forEach(b => {
            ...
            const info = res.results[isbn];
            if (!info) return;

            if (info.thumbnail && !b.thumbnail) {
                b.thumbnail = info.thumbnail;
                toSaveThumb.push({ isbn, url: info.thumbnail });
                updated = true;
            }
            if (info.description && !b.description) {
                b.description = info.description;
                toSaveDesc.push({ isbn, description: info.description });
                updated = true;
            }
        });
        ...
        if (updated) {
            ...
            // サーバー(シート)に書き戻し
            if (toSaveThumb.length) callGAS("saveThumbnailsBulk", toSaveThumb);
            if (toSaveDesc.length) callGAS("saveDescriptionsBulk", toSaveDesc);
        }
    }
}
...
function cleanAuthorName(authors) {
    ...
    // カンマ区切りの生年も含む (e.g. "村上, 春樹, 1949-")
    raw = raw.replace(/[（\(\[\［]\s*\d{4}[-–－—~～〜,]\s*\d*?\s*[）\)\]］]/g, ' ');
    raw = raw.replace(/[,，]?\s*\d{4}[-–－—~～〜]\d*/g, ' ');

    // 年号単体（1984 など）が著者名として残るのを防ぐ
...
async function addBulkISBN(isbn) {
    ...
    if (res && !res.error) {
        ...
        res.author = cleanAuthorName(res.author); // 著者名をクリーンアップ
        bulkQueue.push(res);
        renderBulkList();
    } else {
        toast("⚠️ 取得失敗: " + isbn);
    ...
    if (!m) {
        ...
        <div class="modal" style="max-width:640px">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:12px">
                <select id="search-source" style="padding:10px;font-size:.85rem;background:#f8fafc;color:var(--txt);border-radius:12px;font-weight:700;border:1px solid var(--border)">
                    <option value="both">両方から検索</option>
                    <option value="ndl">国会図書館(NDL)</option>
                    <option value="cinii">CiNii Research</option>
                </select>
                <select id="ndl-type" style="padding:10px;font-size:.85rem;border-radius:12px;background:#f8fafc;border:1px solid var(--border)">
                    <option value="">すべての資料</option>
                    <option value="1">一般図書</option>
                    <option value="4">論文・記事</option>
                    <option value="5">博士論文</option>
                </select>
                <input type="text" id="ndl-query" placeholder="キーワード・タイトル・著者名を入力" style="grid-column:span 2;width:100%;padding:14px;border:1px solid var(--border);border-radius:12px;font-size:1rem">
            </div>
...
function fillFromNDL(idx) {
    ...
    set('f-isbn', r.isbn || '');
    set('f-title', r.title);
    set('f-author', cleanAuthorName(r.author));
...
function cleanAuthorName(authors) {
    ...
    return raw.split(/\s+/)
        .map(a => a.trim())
        .filter(a => a && !/^\d+$/.test(a)) // 数字のみの要素を除去
        .join(' ')
        .replace(/\s+/g, ' ')
        .trim();
}
