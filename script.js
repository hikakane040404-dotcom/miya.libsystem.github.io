...
// 🚨 CONFIG (GASのURL)
// ========================================
const GAS_API_URL = "https://script.google.com/macros/s/AKfycbwgqwMdPmo2MevIeYfwLlopZpmNyb-5wg1DdNnDlZjn6eUoz5DlSc75ogsPZVRrEKKznw/exec";
    ...
    if (!m) {
        ...
        <div class="modal" style="max-width:640px">
            ...
            <div style="display:flex;gap:8px;margin-bottom:10px">
                <select id="search-source" style="width:120px;padding:8px;font-size:.85rem;background:var(--acc);color:#0f172a;border-radius:6px;font-weight:600">
                    <option value="ndl">📖 NDL</option>
                    <option value="cinii">🎓 CiNii</option>
                </select>
                <input type="text" id="ndl-query" placeholder="キーワード・タイトル・論文名・著者名..." style="flex:1">
...
async function doNDLSearch() {
    ...
    let res;
    if (source === 'cinii') {
        res = await callGAS('searchCiNii', { query });
    } else {
        res = await callGAS('searchNDL', { query, mediatype });
    }
