...
// 🚨 CONFIG (GASのURL)
// ========================================
const GAS_API_URL = "https://script.google.com/macros/s/AKfycbw0tQksGc0QyOIf9lSlpMOF0kG-ga234MS97s3MRKKXISLxX5taySUNEO6Fb-eCKyMgOA/exec";
...
function renderBorrowStatus() {
    ...
    // ISBNでグループ化して表示
    const groups = groupBooks(rented);
    c.innerHTML = '';
    groups.forEach(g => {
        const rep = g.rep;
        const borrowers = [...new Set(g.copies.map(b => usersMap[String(b.borrower_id)] || b.borrower_id || '不明'))];
        const d = document.createElement('div');
        d.style.cssText = 'display:flex;align-items:center;gap:10px;padding:10px 12px;background:var(--card);border-radius:10px;border:1px solid var(--border);margin-bottom:8px';
        d.innerHTML =
            `<img src="${getImg(rep)}" style="width:36px;height:52px;object-fit:cover;border-radius:4px;flex-shrink:0">` +
            `<div style="flex:1;min-width:0">` +
            `<div style="font-weight:600;font-size:.88rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${rep.title}</div>` +
            `<div style="font-size:.78rem;color:var(--muted);margin-top:2px">📤 ${borrowers.join(', ')}</div>` +
            `</div>`;
        c.appendChild(d);
    });
}
...
async function updateUserRole(uid, role) {
    ...
    if (res && res.success) toast(`権限を変更しました`);
...
async function deleteUser(uid, name) {
    ...
    if (res && res.success) { toast(`${name} を削除しました`); fetchUsers(); }
    else alert((res && res.message) || "削除に失敗しました");
}

function switchAdminTab(tab) {
    console.log("[Admin] Switching to tab:", tab);
    const booksPanel = document.getElementById('admin-panel-books');
    const usersPanel = document.getElementById('admin-panel-users');
    const booksTabBtn = document.getElementById('admin-tab-books');
    const usersTabBtn = document.getElementById('admin-tab-users');

    if (booksPanel) booksPanel.style.display = tab === 'books' ? 'block' : 'none';
    if (usersPanel) usersPanel.style.display = tab === 'users' ? 'block' : 'none';
    if (booksTabBtn) booksTabBtn.classList.toggle('active', tab === 'books');
...
async function enrichAPI(list) {
    ...
    console.log(`[enrichAPI] サムネイル取得对象: ${targets.length}冊, 説明文取得对象: ${descTargets.length}冊`);
    ...
    for (const b of gbTargets.slice(0, 15)) {
        try {
            ...
            if (d.items && d.items[0]) {
                ...
                if (!b.image_api && v.imageLinks) b.image_api = v.imageLinks.thumbnail.replace(/^http:/, 'https:');
    ...
    if (toSave.length) {
        ...
        callGAS("saveThumbnailsBulk", toSave);
    }
    ...
    if (descToSave.length) {
        console.log(`[enrichAPI] ${descToSave.length}冊の説明文をシートに保存`);
        callGAS("saveDescriptionsBulk", descToSave);
    }
}
...
function renderBooks(f) {
    ...
    groups.forEach(function (g) {
        ...
        d.onclick = function (e) { if (e.target.tagName !== 'BUTTON') openDetail(g.key); };

        // バッジ表示
        let badgeHtml, btnHtml;
        if (avail > 0) {
            badgeHtml = `<span class="badge badge-ok">貸出可 ${avail}/${total}冊</span>`;
            btnHtml = `<button class="btn btn-primary" style="padding:6px 14px;font-size:.82rem" onclick="event.stopPropagation();borrowBook('${firstAvail.id}')">借りる</button>`;
        } else if (isMine) {
            badgeHtml = `<span class="badge badge-ng">貸出中 ${total}/${total}冊</span>`;
            btnHtml = `<button class="btn btn-warn" style="padding:6px 14px;font-size:.82rem" onclick="event.stopPropagation();doReturn('${myBook.id}')">返却</button>`;
        } else {
            badgeHtml = `<span class="badge badge-ng">貸出中 ${total}/${total}冊</span>`;
            btnHtml = `<button class="btn btn-outline" style="padding:6px 14px;font-size:.82rem" onclick="event.stopPropagation();reserveBook('${rep.id}')">予約</button>`;
        }
        ...
        d.innerHTML =
            ...
            `<div class="book-title">${rep.title}${countBadge}</div>` +
            `<div class="book-author">${rep.author || ''} (${rep.publisheddate || rep.publishedDate || '-'})</div>` +
...
function cleanAuthorName(authors) {
    ...
    // 不要な要素をクリーンアップ
    return raw.split(/[;,]/).map(a => String(a)
        .replace(/\s*\(.*?\)/g, '')             // (1920-2000) などを除去
        .replace(/\s*\d{4}[-–－]\d{0,4}/g, '') // 生没年除去 (全角マイナス対応)
        .replace(/[\[\(][著編訳][\]\)]/g, '')   // [著] (編) などを除去
        .replace(/\d{4}年?生/g, '')             // 1980年生 などを除去
        .replace(/\s+/g, ' ')                   // 連続スペース整理
        .trim()
    ).filter(a => a && !/^\d+$/.test(a)).join(' '); // 数字だけの要素は除外
}
    ...
    if (!m) {
        ...
        <div class="modal" style="max-width:640px">
            ...
            <div style="display:flex;gap:8px;margin-bottom:10px">
                <input type="text" id="ndl-query" placeholder="キーワード（タイトル・著者・論文名など）" style="flex:1">
                <select id="ndl-type" style="width:110px;padding:8px;font-size:.85rem">
            ...
            <button class="btn btn-primary" style="width:100%;margin-bottom:14px" onclick="doNDLSearch()">🔍 検索</button>
...
async function doNDLSearch() {
    ...
    if (!res || !res.success) {
        resultsEl.innerHTML = `<p style="text-align:center;color:var(--danger);padding:16px">取得に失敗: ${(res && res.message) || ''}</p>`;
        return;
    }
    renderNDLResults(res.results || []);
}
function renderNDLResults(results) {
    ...
    results.forEach((r, i) => {
        ...
        d.innerHTML =
            `<div style="font-weight:600;font-size:.9rem;margin-bottom:3px">${r.title}${catLabel}</div>` +
            ...
            (r.link ? `<a href="${r.link}" target="_blank" style="font-size:.75rem;color:var(--acc);text-decoration:none">NDLで見る →</a>` : '') +
            `</div>`;
        el.appendChild(d);
    });
}
