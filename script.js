    ...
    if (fIsbn) {
        fIsbn.addEventListener('input', function () {
            ...
            if (v.length >= 10) {
                ...
                isbnDebounceTimer = setTimeout(() => lookupISBN(), 1200);
            }
        });
    }
});
...
function switchAdminTab(tab) {
    document.getElementById('admin-panel-books').style.display = tab === 'books' ? 'block' : 'none';
    document.getElementById('admin-panel-users').style.display = tab === 'users' ? 'block' : 'none';
    document.getElementById('admin-tab-books').classList.toggle('active', tab === 'books');
...
function cleanAuthorName(authors) {
    ...
    return authors.map(a => String(a)
        .replace(/,?\s*\d{4}[-–]\d{0,4}/g, '') // 生没年除去: 1980- / 1980-2020
        .replace(/,\s*/g, ' ')                  // カンマ→スペース
        .replace(/\[.*?\]/g, '')                // [著] [編] などを除去
